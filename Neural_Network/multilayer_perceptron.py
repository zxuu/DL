...
class MultilayerPerceptron:
    ...
    def theta_init(layers):
        '''
        初始化各层神经元
        :param layers: [list] 各层神经元个数
        :return:
        '''
        num_layers = len(layers)    #几层隐含层
        thetas = {}
        for layer_index in range(num_layers-1):    #2次
            """
            会执行两次，得到两组参数：25*785， 10*26
            """
            in_count = layers[layer_index]    #当前层的输入
            out_count = layers[layer_index+1]    #当前层的输出
            #这里需要考虑偏置项，记住一点偏置的个数跟输出的结果是一致的
            thetas[layer_index] = np.random.rand(out_count,in_count+1)*0.05    #随即得到初始化结果，尽量小一点
        return thetas
    ...
    def feedforward_propagation(data, thetas, layers):
        ...
        for layer_index in range(num_layers -1):
            ...
            #正常计算完之后是num_example*25，但是要考虑到偏置项，再加一列1,变成num_example*26
            out_layer_activation = np.hstack((np.ones((num_examples,1)),out_layer_activation))
            in_layer_activation = out_layer_activation
        #返回输出值结果,不要偏置项,[1700,10],10分类
